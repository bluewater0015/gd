...
export default class Owner extends Component{
	...
	typeShowClickEvent(index){
		...
		// })
		//获取不到工单类型
		//console.log("this.state",this.state.typeName);
		...
		//for in 遍历对象 
		//{ workOrderTypeString: '维修',workOrderStatusString: "待派发",city: "城市",}
		for(var x in obj) {
			// x 为对象的键 obj[x] 为键值
		...
		//console.log('obj',obj);
		//console.log('statusNameList',this.state.statusNameList);
		...
		//let typeName= this.state.typeNameList[index];
 	    //console.log('typeName',typeName);
		//过滤,注意从保存的数据中过滤
		// let list = this.state.keepList.filter((item,index)=>{
		// 	return item.workOrderTypeString === typeName
		// })
	...
	statusShowClickEvent(index) {
		...
		//过滤
		// let list = this.state.keepList.filter((item,index)=>{
		// 	return item.workOrderStatusString ===  statusName
		// })
		// if( list.length === 0) {
		// 	this.props.history.push('/noorder');
		// }else {
    ...
    //         return res.json()
    //     }).then(data=>{
    //     	console.log('工单列表数据：',data.content);
    //     	if(data.content.length === 0){
    //     		this.props.history.push(`/noorder`);
    //     	}else {
    //     		let list = data.content;
